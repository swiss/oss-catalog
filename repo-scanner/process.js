...
async function processRepositories() {
    ...
    if (!pasetoToken) {
        ...
        console.error('Please create a .env file and add PASETO_TOKEN="your_token_here"');
    ...
    try {
        ...
        const requestPromises = urls.map((url) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                const req = requestModule.request(options, (res) => {
                    ...
                    res.on('end', () => {
                        if (res.statusCode >= 200 && res.statusCode < 300) {
                            console.log(`Successfully posted ${trimmedUrl}. Status: ${res.statusCode}`);
                            resolve(res.statusCode);
                        } else {
                            console.error(`Failed to post ${trimmedUrl}. Status: ${res.statusCode}, Data: ${responseBody}`);
                ...
                req.on('error', (error) => {
                    console.error(`Error setting up request for ${trimmedUrl}. Error: ${JSON.stringify(error)}`);
                    reject(error);
                });
...
// 1. Save this code as a file, for example, 'process.js'.
// 2. Create a file named 'repos.txt' in the same directory and add your GitHub URLs.
